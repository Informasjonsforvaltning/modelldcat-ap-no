...
Det anbefales at URI til Aktør (`foaf:Agent`) er identisk med den URI som Enhetsregisteret (Brønnøysundregistrene) har på enheten/virksomheten dersom den finnes i registeret, f.eks. `dct:publisher <<https://organization-atalogue.fellesdatakatalog.digdir.no/organizations/974760673>>`.
